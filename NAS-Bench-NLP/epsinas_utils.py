...
def compute_epsinas(x, network, weights, batch_size):
    preds = []
...
def bs_ablation_plots(exp_list, filename):
...
def compute_stats(nas_metric, eval_metric, maximize=True, raw=False):
    # Take care when the evaluation model performance metric should be minimized (perplexity, RMSE)

    # Take care of NaN and null entries for accurate stats calculations
    nas_metric = np.array(nas_metric)
    nas_metric[nas_metric==0] = np.nan
    nonan = ~np.isnan(nas_metric) & ~np.isnan(eval_metric)

    eval_metric_nonan = np.array(eval_metric)[nonan]
    nas_metric_nonan = np.array(nas_metric)[nonan]

    # When working with metrics like accuracy that must be maximized
    if maximize:
        top10top10_ind = (eval_metric_nonan>np.nanpercentile(eval_metric_nonan, 90)) & (nas_metric_nonan>np.nanpercentile(nas_metric_nonan, 90))
        top10model_ind = eval_metric_nonan>np.nanpercentile(eval_metric_nonan, 90)
        top10nas_metric_ind = nas_metric_nonan>np.nanpercentile(nas_metric_nonan, 90)
    # Otherwise, when working with MSE or perplexity
    else:
        top10top10_ind = (eval_metric_nonan<np.nanpercentile(eval_metric_nonan, 10)) & (nas_metric_nonan>np.nanpercentile(nas_metric_nonan, 90))
        top10model_ind = eval_metric_nonan<np.nanpercentile(eval_metric_nonan, 10)
        top10nas_metric_ind = nas_metric_nonan<np.nanpercentile(nas_metric_nonan, 10)

    spearman_all = stats.spearmanr(nas_metric_nonan, eval_metric_nonan, nan_policy='omit')[0]
    spearman_best = stats.spearmanr(nas_metric_nonan[top10model_ind], eval_metric_nonan[top10model_ind], nan_policy='omit')[0]
    kendall_all = stats.kendalltau(nas_metric_nonan, eval_metric_nonan, nan_policy='omit')[0]
    kendall_best = stats.kendalltau(nas_metric_nonan[top10model_ind], eval_metric_nonan[top10model_ind], nan_policy='omit')[0]
    top10top10_frac = np.sum(top10top10_ind)/np.sum(top10nas_metric_ind)*100
    ...
    # Compute stats for the top-64 architectures
    if len(eval_metric_nonan)>64:
        if maximize:
            top64top5_ind = (eval_metric_nonan>np.nanpercentile(eval_metric_nonan, 95)) & (nas_metric_nonan>=np.sort(nas_metric_nonan)[-64])
            outputs_raw.append(sum(top64top5_ind))
        else:
            top64top5_ind = (eval_metric_nonan<np.nanpercentile(eval_metric_nonan, 5)) & (nas_metric_nonan>=np.sort(nas_metric_nonan)[-64])
            outputs_raw.append(sum(top64top5_ind))
    else:
    ...
    if raw:
        return outputs_raw, len(eval_metric_nonan)
    else:
        return outputs, len(eval_metric_nonan)
##

def plot_results(scores, accs, save_dir, save_name, accs_min=None, accs_max=None, nparams=None, top10=False, log_scale=False):
